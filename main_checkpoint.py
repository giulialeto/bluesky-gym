...
if __name__ == "__main__":
    ...
    if TRAIN:
        checkpoint_callback = CheckpointCallback(
            save_freq=100_000,  # save every 100k env steps
            save_path=f"models/{env_name}/{algorithm.__name__}/",
            name_prefix="ckpt"
        )
        ...
        eval_callback = EvalCallback(
            eval_env,
            best_model_save_path=f"models/{env_name}/{algorithm.__name__}/best",
            n_eval_episodes=5,
            eval_freq=50_000
        )
        model.learn(total_timesteps=int(2e6),
            callback=[csv_logger_callback, checkpoint_callback, eval_callback])
